...
data_file = open('data_pasangan_cagub_cawagub.json')
data_pasangan = json.load(data_file)
...
for pasangan in data_pasangan['pasangan_calon']:
    ...
    try:
        cursor.execute(sql_insert_candidate_pair, (cagub_id, cawagub_id,
            pair_name, pair_website_url, pair_logo_url, pair_fbpage_username,
            pair_slogan, pair_description, pair_twitter_username,
            pair_instagram_username))
        db.commit()
    except mysql.Error as err:
        print("Something went wrong: {}".format(err))
        db.rollback()
